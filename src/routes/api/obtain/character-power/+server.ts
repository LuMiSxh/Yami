...
export const POST = (async ({ request, fetch }) => {
	...
	// Fetch item manifest
	const manifestRequest = await fetch(`${PUBLIC_PATH}/api/obtain/manifest`, {
		method: 'POST',
	...
	if (profileItemsRequest.status !== 200) {
		throw error(500, {
			message: `Something went wrong obtaining the items from your Destiny 2 profile: '${
				(await profileItemsRequest.json()).Message
			}'`,
	...
	for (const [charId, char] of Object.entries(armorData)) {
		...
		const leg = char.legs.sort(powerSorter)[0];
